...
export const APP_BUILD_DATE = "25/09/2026 17:55 hs";

export const APP_RELEASE_NOTES: string[] = [
  "Transformación Institucional Provincial (Fase 1: Importador Masivo, Exportador Oficial y Sistema SATDE): 1) Importador Masivo desde Excel/CSV (BulkDataImportModal.tsx y excelService.ts): Se desarrolló el centro de carga masiva de estudiantes y docentes en 'AlumnosTab' y 'ProfesoresTab', permitiendo arrastrar planillas Excel (.xlsx, .xls) o CSV con detección automática de columnas (Nombre, DNI, Curso/Materias, Email), pre-validación fila por fila con semáforo interactivo (listos, duplicados o errores), procesamiento por lotes paralelos en Appwrite y botones de descarga de plantillas oficiales institucionales. 2) Exportador Oficial de Planillas y Actas Institucionales (OfficialDocumentExportModal.tsx y officialPrintService.ts): Se creó el generador ministerial de documentos en alta resolución con membrete formal de la Provincia del Chubut y la Escuela N° 713 'Juan Abdala Chayep' (CUE: 2600214-00). Permite emitir e imprimir o guardar en PDF: Planillas Mensuales de Asistencia (cuadrícula oficial días 1 a 31 por curso con cómputo de faltas, porcentajes y casilleros de firma para Preceptoría, Secretaría y Dirección), Actas Volantes Oficiales de Examen (con campos de Libro Matriz, Folio, Tribunal examinador, notas escritas/orales y condición final) y Padrón Oficial de Alumnos, con exportación complementaria a formato Excel (.xlsx). 3) Sistema de Alerta Temprana de Deserción Escolar - SATDE (EarlyWarningDropoutWidget.tsx y MonitorAsistenciaTab.tsx): Se diseñó e integró el módulo de inteligencia preventiva contra el abandono escolar basado en la normativa del Ministerio de Educación de Chubut. Clasifica en tiempo real a los estudiantes en 4 categorías reglamentarias (Regularidad Normal <10 faltas, Alerta Preventiva 10-14.5 faltas, Riesgo Crítico 15-19.5 faltas y Pérdida de Regularidad ≥20 faltas) con barras de proximidad al límite reglamentario y generación en 1 clic de la Cédula Oficial de Citación a Padres con talón de constancia y firma para Preceptoría. 4) Acceso Rápido en Barra Superior (TopNavSidebar.tsx y page.tsx): Se incorporó el acceso directo 'Monitor y Alerta Temprana (SATDE)' en la barra de navegación superior con icono vectorial temático y sincronización instantánea."
];

export const APP_VERSION_HISTORY: VersionItem[] = [
  {
    version: "v2.27.3",
    date: "25/09/2026 17:15 hs",
    notes: [
  ...
  },
  {
    ...
    notes: [
      "Auditoría y Blindaje de Seguridad Integral (Next.js 16.3.6, rateLimit.ts, send-code, verify-code-reset, reject-user y send-email): Se llevó a cabo un escaneo y remediación de seguridad de extremo a extremo en la plataforma. 1) Actualización Crítica de Dependencias: Se actualizó Next.js y eslint-config-next a la versión parcheada 16.3.6, mitigando todas las vulnerabilidades críticas y altas reportadas por GitHub Advisory (GHSA-8h8q-6873-q5fj, GHSA-26hh-7cqf-hhc6, bypass de middleware, DoS en Server Actions y exposición de endpoints). 2) Protección contra Fuerza Bruta y Spam (rateLimit.ts): Se diseñó e implementó un gestor de Rate Limiting en memoria para frenar ataques automatizados en endpoints sensibles. En '/api/auth/send-code', se impuso un tope de solicitudes por IP y casilla; en '/api/auth/verify-code-reset', se bloquearon los intentos reiterados de adivinación de códigos OTP y se activó el consumo de token de un solo uso (Single-Use Token) para impedir ataques de repetición (Replay Attacks). 3) Autorización Estricta en Rechazo de Usuarios (/api/admin/reject-user): Se incorporó verificación obligatoria de credenciales y comprobación en servidor de que el operador posea rol de Administrador o Directivo antes de autorizar cualquier purga de cuentas o documentos. 4) Mitigación de Open Mail Relay y CSRF (/api/send-email): Se blindó el servicio de correos con validación de Same-Origin, sanitización estricta de encabezados (Subject Header Injection) y topes de destinatarios concurrentes. 5) Verificación de Gmail Previa al Registro y Componente OTP con Microinteracciones (OtpVerificationInput.tsx, page.tsx, verify-code/route.ts y UserProfileModal.tsx): Se añadió la verificación obligatoria de casilla de correo mediante código OTP de 6 dígitos antes de completar la creación de cuenta oficial. Se integró el componente interactivo OtpVerificationInput (impulsado por 'input-otp' y físicas elásticas de 'motion'), ofreciendo casillas segmentadas, auto-avance inteligente, soporte de pegado rápido (Ctrl+V), cursor pulsante y feedback de éxito/error, unificando la experiencia en registro, recuperación de contraseña y cambio de clave en el perfil de usuario."
    ]
  },
  {
    version: "v2.26.1",
    date: "24/09/2026 22:48 hs",
    notes: [
